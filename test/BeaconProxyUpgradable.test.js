...
contract('ProductBeaconProxy', function (accounts) {
  ...
  it("Proxy with newer implementation should be able to call initialize function from older implementations", async function(){
    await this.beacon.upgradeTo(this.implementationV1.address);
  ...
  xit('Redeem flow check', async function (){
    const DEG = true;
  ...
  it('multi Token Pricing Functions update',async function(){
    const data = this.implementationV0.contract.methods.initialize('HighGO', 'HG', exp, max, offset, baseReserve).encodeABI();
      await this.tokenFactory.createToken(
        "HighGO", data,
      );
  ...
  it('Availability  Functions  update',async function(){
    ...
    const totalSupplyV2 = await highGoV2.getAvailability()
		...
		totalSupplyV1.should.be.a.bignumber.that.equals(totalSupplyV2)
  });

  xit('Skip V2 version  rollback calculate Sale Return  Functions  update', async function(){
    const data = this.implementationV0.contract.methods.initialize('HighGO', 'HG', exp, max, offset, baseReserve).encodeABI();
      await this.tokenFactory.createToken(
        "HighGO", data,
      );
  ...
  xit('Destroy ProductToken does not succeed', async function(){
  ...
  xit('update new initialize by implementationV4 ',async function(){   
    this.beacon.upgradeTo(this.implementationV1.address);
    this.beacon.upgradeTo(this.implementationV2.address);
    this.beacon.upgradeTo(this.implementationV3.address);
    ...
    const dataV5 = this.implementationV4.contract.methods.initialize('HighGO', 'HG', exp, max, offset, baseReserve).encodeABI();
      await this.tokenFactory.createToken(
        "HighGO", dataV5,
      );
      const proxyAddress = await this.tokenFactory.retrieveToken.call("HighGO");
